...
demographic = pd.read_csv(
    '/home/paul/thesis/dev/SAY_sf_prediction_v2/dataset/demographic.csv')
...
for i in [25, 26]:  # np.arange(100):
    # 'gqi_count_sum', 'msmt_sift_radius2_count', 'dti_dti_count':
    for edge_weight in ['gqi_count_sum']:
        with open('/home/paul/thesis/dev/SAY_sf_prediction_v2/dataset/preds_percent_' + atlas + '_' + edge_weight + '_diag1_n' + str(i) + '.pkl', 'rb') as f_pred:
            ...
            # this needs to be adapted based on how the pickle loads the dictionary of lists, which walk lengths are included, etc.
            pred_FC = preds[4]['test']
        ...
        fc_test = h5py.File('/home/paul/thesis/dev/SAY_sf_prediction_v2/dataset/test_percent_' +
                            edge_weight + '_diag1_' + str(i) + '.h5py', 'r')
        fc_test = list(np.array(fc_test.get('labels')))
        # get the file_names for each subject
        file_names_fc_test = h5py.File(
            '/home/paul/thesis/dev/SAY_sf_prediction_v2/dataset/test_percent_' + edge_weight + '_diag1_' + str(i) + '.h5py', 'r')
        file_names_fc_test = list(
            np.array(file_names_fc_test.get('file_name')))
        # set up empty array to store flattened upper triangle of fc_test matrices for each subject
        # 6670 is the number of edges in the upper triangle of a 116x116 matrix
        FC_test = np.zeros((len(fc_test), 6670))
        ...
        # load empirical functional connectivity data from h5py
        fc_train = h5py.File('/home/paul/thesis/dev/SAY_sf_prediction_v2/dataset/train_percent_' +
                             edge_weight + '_diag1_' + str(i) + '.h5py', 'r')
        fc_train = list(np.array(fc_train.get('labels')))
        # get the file_names for each subject
        file_names_fc_train = h5py.File(
            '/home/paul/thesis/dev/SAY_sf_prediction_v2/dataset/train_percent_' + edge_weight + '_diag1_' + str(i) + '.h5py', 'r')
        file_names_fc_train = list(
            np.array(file_names_fc_train.get('file_name')))
        # set up empty array to store flattened upper triangle of fc_test matrices for each subject
        # 6670 is the number of edges in the upper triangle of a 116x116 matrix
        FC_train = np.zeros((len(fc_train), 6670))
        # get flattened upper triangle of fc_test matrices for each subject
        ...
        behavioral['clinical_outcome'] = np.random.randint(
            0, 2, behavioral.shape[0])
        ...
        df_summary.to_csv('/home/paul/thesis/dev/SAY_sf_prediction_v2/hierarchical_model_summary_' +
                          edge_weight + '_n' + str(i) + '.csv')
        # Print the summary of the posterior distribution
        pm.summary(trace)
        # Plotting posterior distributions
        # , credible_interval=0.95) # this threw an error for unexpected argument credible_interval
        pm.plot_posterior(trace["alpha"])
        plt.xlabel("Alpha")
        plt.ylabel("Density")
        plt.title("Posterior Distribution of Alpha")
        # save figure with edge_weight and dataset number in the filename
        plt.savefig('/home/paul/thesis/dev/SAY_sf_prediction_v2/hierarchical_model_posterior_alpha_' +
                    edge_weight + '_dataset_n' + str(i) + '.png')
        plt.show()
        plt.close()
