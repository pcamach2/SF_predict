# usage: test_sf_prediction_riemannian_msmt_percent_save_v5.py <sc_connectomes_dir> <functional_connectomes_dir> <no_scramble>
import os
import numpy as np
...
for atlas in atlases:
    ...
    for edge_weight in ['sift_radius2_count', 'sift_invnodevol_radius2_count', 'radius2_count', 'radius2_meanlength']:
        ...
        for iii in np.arange(100):
            ...
            # Write
            f_train = h5py.File('/datain/dataset/train_percent_msmt_' +
            ...
            f_train.close()
            f_test = h5py.File('/datain/dataset/test_percent_msmt_' +
                          edge_weight + '_diag1_' + str(iii) + '.h5py', 'w')
            ...
            f_test.close()
            if not no_scramble:
                f_train_scrambled = h5py.File('/datain/dataset/train_scrambled_percent_msmt_' +
                                              edge_weight + '_diag1_' + str(iii) + '.h5py', 'w')
                ...
                f_test_scrambled = h5py.File('/datain/dataset/test_scrambled_percent_msmt_' +
                                             edge_weight + '_diag1_' + str(iii) + '.h5py', 'w')
