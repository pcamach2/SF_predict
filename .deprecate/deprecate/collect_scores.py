...
def collect_scores(data1, data2, data3, column_id, weight_type):
    ...
    df.to_csv(
        f"/home/paul/thesis/dev/SAY_sf_prediction_v2/dataset/{weight_type}_all_batch{column_id}.csv")


# set path for data - this is /home/paul/thesis/dev/SAY_sf_prediction_v2/dataset for mounting to docker/singularity container
...
files_mean_path_length = Path(main_path).glob(
    'scores*dti*mean*diag1*clean*csv')
files_volume_weighted_count = Path(main_path).glob(
    'scores*aal116_dti_dti_volumeweighted_diag1_???_clean.csv')
files_count = Path(main_path).glob(
    'scores*_aal116_dti_dti_count_diag1_???_clean.csv')
...
files_sift_radius2_count = Path(main_path).glob(
    'scores*msmt_sift_radius2_count_diag1*clean*csv')
files_sift_radius2_invnodevol_count = Path(main_path).glob(
    'scores*msmt_sift_invnodevol_radius2_count_diag1*clean*csv')
files_radius2_count = Path(main_path).glob(
    'scores*msmt_radius2_count_diag1*clean*csv')
files_radius2_meanlength = Path(main_path).glob(
    'scores*msmt_radius2_meanlength_diag1*clean*csv')
