...
for atlas in atlases:
    ...
    for edge_weight in 'count_sum', 'ncount_sum', 'mean_length_sum', 'gfa_sum':
        for iii in np.arange(25):
            ...
            # save preds to disk
            with open('/datain/dataset/preds_' + atlas + '_gqi_' + edge_weight + '_diag1_n' + str(ii) + '.pkl', 'wb') as fp:
            ...
            f = h5py.File('/datain/dataset/train_scrambled_gqi_' +
                          edge_weight + '_diag1_' + str(ii) + '.h5py', 'r')
            sc_train = list(np.array(f.get('inputs')))
            fc_train = list(np.array(f.get('labels')))
            f.close()
            f = h5py.File('/datain/dataset/test_scrambled_gqi_' +
                          edge_weight + '_diag1_' + str(ii) + '.h5py', 'r')
            sc_test = list(np.array(f.get('inputs')))
            fc_test = list(np.array(f.get('labels')))
            f.close()
